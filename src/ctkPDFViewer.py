...
class ShowPdf():
    ...
    def pdf_view(self,master, frame=None,width=1200,height=600,pdf_location="",bar=True,load="after"):
        self.master = master
        self.frame = CTkFrame(frame,width= width,height= height,bg_color="transparent")
        self.frame.pack(fill="both", side="top")
        scroll_y = CTkScrollbar(frame,orientation="vertical")
        scroll_x = CTkScrollbar(frame,orientation="horizontal")
        ...
        if bar==True and load=="after":
            ...
            self.display_msg.pack(pady=15)

            loading = Progressbar(self.frame,orient= HORIZONTAL,length=100,mode='determinate')
            loading.pack(side = TOP,fill=X)

        text = Text(self.frame,yscrollcommand=scroll_y.set,xscrollcommand= scroll_x.set,width= width,height= height, bg=master.cget("bg"))
        text.pack(side="left")

        scroll_x.configure(command=text.xview)
        scroll_y.configure(command=text.yview)
        ...
        def add_img():
            ...
            for page in open_pdf:
                ...
                if bar==True and load=="after":
                    precentage_dicide = precentage_dicide + 0.15
                    percentage_view = (float(precentage_dicide)/float(len(open_pdf))*float(100))
                    loading['value'] = percentage_view
                    percentage_load.set(f"Please wait!, your pdf is loading {int(math.floor(min(100, percentage_view*7)))}%")
            if bar==True and load=="after":
                loading.pack_forget()
                self.display_msg.pack_forget()

            for page in self.img_object_li:
                text.image_create(END,image=page)
                text.insert(END,"\n\n")
