...
def save_image(photo_image, directory):
  """Saves a tkinter.PhotoImage object to a specific directory."""

  # Get the PIL Image object from the tkinter PhotoImage object.
  pil_image = ImageTk.getimage(photo_image)

  # Save the PIL Image object to the specified directory.
  pil_image.save(f"{directory}/{photo_image.name}.png")
...
class PDFViewer:
    ...
    def __init__(self, master):
        ...
        # creating a select page button
        self.selectpage = ttk.Button(self.bottom_frame, command=self.select_page)
        self.selectpage.grid(row=0, column=5, pady=8)
    ...
    def next_page(self):
        ...
        if self.fileisopen:
            ...
            if self.current_page <= self.numPages - 1:
                ...
                self.display_page()
                print(self.miner.get_text(self.current_page))
    ...
    def select_page(self):
        if self.fileisopen:
            if 0 <= self.current_page < self.numPages:
                image = self.miner.get_page(self.current_page)
                path = folder_path = fd.askdirectory(initialdir=".", title="Select a folder to save the image")
                if path:
